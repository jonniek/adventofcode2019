...
impl VM {
  ...
  pub fn output_ascii(&mut self) -> Option<i64> {

    while let Some(out) = read_output(self.read_input(None)) {
      // If output is not ascii return it
      if out > u8::max_value() as i64 {
        return Some(out);
      }
      let c: char = (out as u8).into();
      print!("{}", c);
    }
  ...
  pub fn input_ascii(&mut self, input: &str) {
    for ch in input.chars() {
      let out = self.read_input(Some(ch as i64));
      match out {
        Output::Value(value) => {
